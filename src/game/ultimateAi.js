import { calculateWinner, getAvailableMoves, otherSymbol } from './gameLogic.js'
...
/**
 * Scores every legal (board, cell) move: local wins/blocks, positional value,
 * and — on higher difficulty — how safe the resulting hand-off is for the
...
function scoredMove(state, aiSymbol, { sendSafety }) {
  ...
  for (const move of moves) {
    ...
    if (subResult.winner === aiSymbol) {
      ...
      if (calculateMetaWinner(nextBoardWinners).winner === aiSymbol) score += 1000
    } else {
      const humanTest = subCells.slice()
      humanTest[move.cell] = humanSymbol
      if (calculateWinner(humanTest).winner === humanSymbol) score += 45 // blocks a live threat
    }
    ...
    if (sendSafety) {
      ...
      if (!targetClosed) {
        const targetCells = move.board === sendTarget ? nextSub : state.boards[sendTarget]
        if (canWinBoardNextTurn(targetCells, humanSymbol)) score -= 55
      } else {
        score -= 8 // handing over free choice is mildly risky
      }
    }
...
/**
 * @param {ReturnType<typeof import('./ultimateLogic.js').createUltimateState>} state
 * @param {'X'|'O'} aiSymbol
...
export function getUltimateAiMove(state, aiSymbol, difficulty) {
  ...
  if (difficulty === 'medium') return scoredMove(state, aiSymbol, { sendSafety: false })
