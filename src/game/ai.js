import { calculateWinner, getAvailableMoves, isBoardFull, otherSymbol } from './gameLogic.js'
...
/** Returns a move that immediately wins for `symbol`, or null. */
...
/** Wins if it can, blocks if it must, otherwise plays positionally — beatable but sharp. */
...
function minimax(cells, symbolToMove, aiSymbol, depth, alpha, beta) {
  ...
  for (const move of getAvailableMoves(cells)) {
    ...
    const score = minimax(next, otherSymbol(symbolToMove), aiSymbol, depth + 1, alpha, beta)
...
/** Perfect play via minimax + alpha-beta pruning. Cannot be beaten, only drawn. */
function getHardMove(cells, aiSymbol) {
  ...
  for (const move of getAvailableMoves(cells)) {
    const next = cells.slice()
    next[move] = aiSymbol
    const score = minimax(next, otherSymbol(aiSymbol), aiSymbol, 1, -Infinity, Infinity)
  ...
  // Break ties randomly so "unbeatable" doesn't also mean "predictable".
