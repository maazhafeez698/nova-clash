  ...
  return (
    <div
      ...
      className={[
        ...
        isActive
          ? "border-cyan/60 bg-void-surface shadow-glow"
          : "border-void-line bg-void-surface/60",
        metaHighlight
      ...
      {cells.map((value, i) => {
        ...
        return (
          <button
            ...
            aria-label={
              value
                ? `board ${boardIndex + 1} cell ${i + 1}, marked ${value}`
                : `board ${boardIndex + 1} cell ${i + 1}, empty`
            }
            tabIndex={clickable ? 0 : -1}
            className={[
              "aspect-square rounded-sm flex items-center justify-center bg-void-deep/60",
              clickable
                ? "hover:bg-void-raised cursor-pointer"
                : "cursor-default",
            ].join(" ")}
          >
